        ...
        results.append(
            make_data_dict(
                ...
                cur_conversations=[
                    ...
                    f"根据用户输入问题，编写sql代码如下：\n```sql\n SELECT t.统计截止日期, t.证券代码, b.股票简称, t.持股比例, b.行业名称 FROM (SELECT a.统计截止日期, a.证券代码, a.持股比例 FROM 十大股东 AS a JOIN 上市公司基本信息年度表 AS b ON (a.统计截止日期=b.统计截止日期 AND (a.股东名称=b.中文全称 OR a.股东名称=b.股票简称) AND (b.股票简称='{data[i]}' OR b.股票代码='{data[i]}' OR b.股票代码='{data[i][0:6]}') AND a.持股比例>0.3)) AS t JOIN 上市公司基本信息年度表 AS b ON (t.统计截止日期=b.统计截止日期 AND t.证券代码=b.股票代码) ORDER BY t.统计截止日期 ASC AND t.持股比例 DESC;  \n```"
                ]
            )
        )
