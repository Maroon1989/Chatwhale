        ...
        results.append(
            make_data_dict(
                ...
                cur_conversations=[
                    ...
                    f"根据用户输入问题，编写sql代码如下：\n```sql\n SELECT t.统计截止日期, t.证券代码, b.股票简称, t.持股比例, b.行业名称 FROM (SELECT a.统计截止日期, a.证券代码, a.持股比例 FROM 十大股东 AS a JOIN 上市公司基本信息年度表 AS b ON (a.统计截止日期=b.统计截止日期 AND (a.股东名称=b.中文全称 OR a.股东名称=b.股票简称) AND (b.股票简称={data[i]} OR b.股票代码={data[i]} OR b.股票代码={data[i][0:6]}) AND a.持股比例>0.3)) AS t JOIN 上市公司基本信息年度表 AS b ON (t.统计截止日期=b.统计截止日期 AND t.证券代码=b.股票代码) ORDER BY t.统计截止日期 ASC AND t.持股比例 DSC;  \n```"
                ]
            )
        )
        ...
        results.append(
            make_data_dict(
                ...
                cur_conversations=[
                    ...
                    f"根据用户输入问题，编写sql代码如下：\n```sql\n SELECT t.统计截止日期, t.证券代码, b.股票简称, t.持股比例, t1.流动比率, t1.速动比率, t1.营运资金, t1.利息保障倍数, t1.资产负债率, t1.权益乘数, t2.可持续增长率, t3.净资产收益率（ROE）A, t4.应收账款周转率A, t4.存货周转率A, t4.应付账款周转率 FROM (SELECT a.统计截止日期, a.证券代码, a.持股比例 FROM 十大股东 AS a JOIN 上市公司基本信息年度表 AS b ON (a.统计截止日期=b.统计截止日期 AND (a.股东名称=b.中文全称 OR a.股东名称=b.股票简称) AND (b.股票简称={data[i]} OR b.股票代码={data[i]} OR b.股票代码={data[i][0:6]}) AND a.持股比例>0.3)) AS t JOIN 上市公司基本信息年度表 AS b ON (t.统计截止日期=b.统计截止日期 AND t.证券代码=b.股票代码) JOIN 偿债能力 AS t1 ON (t.统计截止日期=t1.统计截止日期 AND t.证券代码=t1.股票代码) JOIN 发展能力 AS t2 ON (t.统计截止日期=t2.统计截止日期 AND t.证券代码=t2.股票代码) JOIN 盈利能力 AS t3 ON (t.统计截止日期=t3.统计截止日期 AND t.证券代码=t3.股票代码) JOIN 经营能力 AS t4 ON (t.统计截止日期=t4.统计截止日期 AND t.证券代码=t4.股票代码) ORDER BY t.统计截止日期 ASC AND t.持股比例 DSC;  \n```"
                ]
            )
        )
        ID_COUNTER.increment()
