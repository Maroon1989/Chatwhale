...
from configs import JSON_SAVE_PATH_CLASSIFICATION,JSON_SAVE_PATH_KEYWORDS,JSON_SAVE_PATH_NL2SQL
...
def save_results(data, types,data_id2file_name) -> None:
    # try:
    ...
    try:
        ...
        existing_data = []
    existing_data.extend(data_classification)
    ...
    try:
        ...
        existing_data = []
    existing_data.extend(data_keywords)
    with open(os.path.join(JSON_SAVE_PATH_KEYWORDS, 'data.json'), 'w', encoding='utf-8') as json_file:
        json.dump(existing_data, json_file, ensure_ascii=False, indent=4)
...
def get_command_line_parser():
    ...
    parser.add_argument('--exp', type=str, default='stock_list')
